    ...
    <>
      <div className="p-4 bg-background border-t border-border flex justify-center">
        <form onSubmit={handleSendMessage} className="relative max-w-4xl w-full">
          <textarea
            className="w-full bg-transparent border border-border rounded-2xl p-4 pr-14 text-foreground placeholder:text-muted-foreground max-h-40 overflow-y-auto resize-none focus:ring-2 focus:ring-cyan-400 focus:outline-none glass-effect disabled:opacity-50"
            placeholder={isDisabled ? t('chatInput.disabledPlaceholder') : t('chatInput.placeholder')}
            value={prompt}
            onChange={(e) => setPrompt(e.target.value)}
            onKeyDown={handleKeyDown}
            disabled={isDisabled || !user || isSendingMessage}
          ></textarea>
          <button type="submit" className="absolute right-4 bottom-4 h-10 w-10 flex items-center justify-center rounded-full bg-cyan-500 text-black disabled:bg-gray-600" disabled={isDisabled || !prompt.trim() || isSendingMessage || !user}>
