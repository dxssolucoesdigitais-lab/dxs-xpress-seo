...
const PricingDialog: React.FC<PricingDialogProps> = ({ isOpen, onOpenChange }) => {
  ...
  const handleSelectPlan = async (planId: string) => {
    ...
    try {
      ...
      if (error.context && error.context.json && error.context.json.error === 'International payments not yet supported.') {
        showError('toasts.plans.internationalPaymentsNotSupported');
      } else {
